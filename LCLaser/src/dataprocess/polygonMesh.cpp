...
****************************/
PolygonMesh::Vertex* PolygonMesh::newVertex(const Vec3D& p)
{
    /* Create a new vertex and link it to the vertex list: */
    ++numVertices;

    Vertex* newVertex=new(vertexAllocator.allocate()) Vertex(p,RPColor(0.3,0.3,0.3,0.3),RPUV(0,0),0);
...
PolygonMesh::Vertex* PolygonMesh::newVertex(const Vec3D& p,const RPColor& c)
{
    /* Create a new vertex and link it to the vertex list: */
    ++numVertices;

    Vertex* newVertex=new(vertexAllocator.allocate()) Vertex(p,c,RPUV(0,0),0);
    newVertex->version=version;
    newVertex->pred=lastVertex;
...
PolygonMesh::Vertex* PolygonMesh::newVertex(const Vec3D& p,const RPColor& c,const ::RPUV &uv)
 {
    /* Create a new vertex and link it to the vertex list: */
    ++numVertices;
    Vertex* newVertex=new(vertexAllocator.allocate()) Vertex(p,c,uv,0);
    newVertex->version=version;
    newVertex->pred=lastVertex;
    if(lastVertex!=0)
        lastVertex->succ=newVertex;
...
PolygonMesh::Edge* PolygonMesh::newEdge()
{
    /* Create a new edge: */
    ++numEdges;
...
PolygonMesh::Face* PolygonMesh::newFace()
{
     ...
     numFaces(0),faces(0),lastFace(0)
{
    ...
    HashTable<const Vertex*,Vertex*> vertexMap((source.getNumVertices()*3)/2);
    for(const Vertex* vPtr=source.vertices;vPtr!=0;vPtr=vPtr->succ)
        vertexMap.setEntry(HashTable<const Vertex*,Vertex*>::Entry(vPtr,newVertex(*vPtr,vPtr->color,vPtr->uv)));
...
PolygonMesh::EdgeHasher* PolygonMesh::startAddingFaces()
    {
...
void PolygonMesh::calFacesZ()
{
    double maxZ,minZ,curZ;
    Vertex *vertex[3];
    Line line[2];
    int i = 0;
    Vec3D head,tail,faceNormal;

    for(PolygonMesh::FaceIterator fIt= beginFaces(); fIt!= endFaces(); ++fIt)
    {
        i = 0;
        for(FaceEdgeIterator feIt = fIt.beginEdges(); feIt!=fIt.endEdges(); ++feIt)
        {
             if(feIt == fIt.beginEdges())
             {
                    vertex[0] = feIt->getStart();
                    maxZ = vertex[0]->getZ();
                    minZ = maxZ;
             }
             else
             {
                    vertex[i] = feIt->getStart();
                    curZ = vertex[i]->getZ();
                    if(curZ > maxZ)
                        maxZ = curZ;
                    if(curZ < minZ)
             ...
             Vec2D vec2(minZ,maxZ);
             fIt->setRangeZ(vec2);

             head = *vertex[0];
             tail = *vertex[1];
             line[0] = Line(head, tail);
             head = tail;
             tail = *vertex[2];
             line[1] = Line(head,tail);
             faceNormal = line[0].Cross3D(line[1]);
             fIt->setNormal(faceNormal);

              for(FaceEdgeIterator feIt = fIt.beginEdges(); feIt!=fIt.endEdges(); ++feIt)
              {
                feIt->getStart()->normal = faceNormal;
              }
        }
}


void PolygonMesh::setFacesTraveType(short sv)
{
    if(sv < 0 || sv >2)
        return;
...
void PolygonMesh::alterVertices(Matrix4X4 mat)
{
    for(Vertex *vPtr = vertices; vPtr!=0; vPtr = vPtr->succ)
    {
        Vec3D vec3;
        vec3.setX(vPtr->getX()); vec3.setY(vPtr->getY()); vec3.setZ(vPtr->getZ());
        vec3 = mat*vec3;
        vPtr->setX(vec3.getX()); vPtr->setY(vec3.getY()); vPtr->setZ(vec3.getZ());
    }
    updateVertexNormals();
}
...
void PolygonMesh::flipNormals()
    {
       for(Vertex* vPtr=vertices;vPtr!=0;vPtr=vPtr->succ)
        if(vPtr->version==version&&vPtr->getEdge()!=0)
            ...
            vPtr->normal.setX(-vPtr->normal.getX());
            vPtr->normal.setY(-vPtr->normal.getY());
            vPtr->normal.setZ(-vPtr->normal.getZ());
            }
            cout<<"======================================"<<std::endl;
    }

float PolygonMesh::getKey()
    {
       float keys = 0;
       int counter = 0;
       for(Vertex* vPtr=vertices;vPtr!=0;vPtr=vPtr->succ)
...
void PolygonMesh::updateVertexNormals()
    {
    ...
    for(Vertex* vPtr=vertices;vPtr!=0;vPtr=vPtr->succ)
        if(vPtr->version==version&&vPtr->getEdge()!=0)
            ...
            do
                ...
                vec1 = (*ve->getEnd()-*vPtr);
                vec2 = (*ve2->getStart()-*vPtr);
                vPtr->normal+= vec1.Cross(vec2);

                /* Go to next edge around vertex: */
                ve=ve2->getOpposite();
                }
            while(ve!=0&&ve!=vPtr->getEdge());
            vPtr->normal.Normalize();//adding
            ...
            if(ve==0) // The vertex' platelet is open
                ...
                while(ve!=0)
                    ...
                    vec1 = (*ve2->getEnd()-*vPtr);
                    vec2 = (*ve->getStart()-*vPtr);
...
PolygonMesh::FaceIterator PolygonMesh::splitFaceDooSabin(const PolygonMesh::FaceIterator& faceIt)
    ...
    Vec3D centroid=centroidC.getPoint();
    RPColor centroidColor=centroidC.getColor();
    ...
    for(int i=0;i<numVertices;++i,outerEdge=outerEdge->getFaceSucc())
        ...
        Vec3D newPoint=Mid(centroid,*outerEdge->getStart());
        RPColor newColor;
        for(int i=0;i<4;++i)
            newColor.rgba[i]=GLubyte(floor((centroidColor.rgba[i]+GLfloat(outerEdge->getStart()->color.rgba[i]))*0.5f+0.5f));
